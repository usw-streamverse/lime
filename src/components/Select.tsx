...
const Select = (props: {width: string, value: React.MutableRefObject<number>, option: {name: string, value: any}[]}) => {
  const [selected, setSelected] = useState(0);
  const [show, setShow] = useState(false);

  const Click = (index: number) => {
    if(props.value)
      props.value.current = props.option[index].value;
    setSelected(index);
    setShow(false);
  }
  ...
  useEffect(() => {
    if(props.value)
      props.value.current = props.option[0].value;
  }, [props.value, props.option])

  return (
    <SelectBox style={{width: props.width}} show={show}>
      <div onClick={() => setShow(!show)}>{props.option.length > 0 ? props.option[selected].name : ''}</div>
      <ul style={{display: show ? 'block' : 'none'}}>
        {
          props.option.map((i, index) => {
            return (
              <li key={index} onClick={() => Click(index)}>{i.name}</li>
            )
          })
        }
      </ul>
